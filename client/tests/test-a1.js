window.addEventListener("load", () => {
	...
	const tests = [
		{
			...
			maxScore: 6,
			run: async () => {
				let result = { id: 1, score: 0, comments: [] };
					...
					if (!appMenu) {
						...
						if (items.length !== 2) {
							result.comments.push(
								printError("Could not find 2 li.menu-item elements")
							);
						} else {
							result.score += 1;
						...
						if (!profileLink) {
							result.comments.push(
								printError("Could not find link to /profile")
							);
						} else {
							result.score += 1;
		...
		},
		{
			...
			maxScore: 6,
			run: async () => {
				let result = { id: 2, score: 0, comments: [] };
				if (
					window.location.pathname !== "/" &&
					window.location.pathname !== "/index.html"
				) {
					result.comments.push(
						print("Test only applicable in index.html page")
					);
					return result;
				}
				let appView = document.querySelector("div#app-view");
					...
					if (!pageView) {
						...
						if (!pageContent) {
							...
							if (!roomList) {
								...
								if (!chatLink) {
									result.comments.push(
										printError("Could not find link to /chat")
									);
								} else {
									result.score += 1;
							...
							if (!pageControl) {
								result.comments.push(
									printError("Could not find div.page-control")
								);
							} else {
								result.score += 1;
								...
								if (!roomInput) {
									result.comments.push(
										printError("Could not find text input in div.page-control")
									);
								} else {
									result.score += 1;
								...
								if (!roomBtn) {
									result.comments.push(
										printError("Could not find button in div.page-control")
									);
								} else {
									result.score += 1;
		...
		},
		{
			...
			maxScore: 10,
			run: async () => {
				let result = { id: 3, score: 0, comments: [] };
				if (
				...
				if (!appView) {
					...
					if (!pageView) {
						...
						if (!pageContent) {
							...
							if (!roomName) {
								result.comments.push(printError("Could not find h4.room-name"));
							} else {
								result.score += 1;
							...
							if (!messageList) {
								result.comments.push(
									printError("Could not find div.message-list")
								);
							} else {
								result.score += 1;
								printOK("Found div.message-list");
								let otherMessage = messageList.querySelector(
									".message:not(.my-message)"
								);
								if (!otherMessage) {
									result.comments.push(
										printError(
											"Could not find at least 1 div.message that is not div.my-message"
										)
									);
								} else {
									result.score += 1;
									...
									let otherUsername =
										otherMessage.querySelector("span.message-user");
									if (!otherUsername) {
										result.comments.push(
											printError(
												"Could not find span.message-user inside div.message"
											)
										);
									} else {
										result.score += 0.5;
									...
									let otherText =
										otherMessage.querySelector("span.message-text");
									if (!otherText) {
										result.comments.push(
											printError(
												"Could not find span.message-text inside div.message"
											)
										);
									} else {
										result.score += 0.5;
								...
								if (!myMessage) {
									result.comments.push(
										printError("Could not find at least 1 div.my-message")
									);
								} else {
									result.score += 1;
									...
									if (!myUsername) {
										result.comments.push(
											printError(
												"Could not find span.message-user inside div.my-message"
											)
										);
									} else {
										result.score += 0.5;
									...
									if (!myText) {
										result.comments.push(
											printError(
												"Could not find span.message-text inside div.my-message"
											)
										);
									} else {
										result.score += 0.5;
							...
							if (!pageControl) {
								...
								if (!chatInput) {
									result.comments.push(
										printError("Could not find textarea in div.page-control")
									);
								} else {
		...
		},
		{
			...
			maxScore: 6,
			run: async () => {
				let result = { id: 4, score: 0, comments: [] };
				if (
					window.location.pathname !== "/profile.html" &&
					window.location.pathname !== "/profile"
				) {
					result.comments.push(
						print("Test only applicable in profile.html page")
					);
					return result;
				}
				let appView = document.querySelector("div#app-view");
				if (!appView) {
					...
					if (!pageView) {
						...
						if (!pageContent) {
							...
							if (!form) {
								result.comments.push(
									printError("Could not find div.profile-form")
								);
							} else {
								result.score += 1;
								...
								if (fields.length < 1) {
									result.comments.push(
										printError(
											"Could not find at least 1 div.form-field inside div.profile-form"
										)
									);
								} else {
									result.score += 1;
									...
									if (!fieldLabel) {
										result.comments.push(
											printError("Could not find label inside div.form-field")
										);
									} else {
										result.score += 0.5;
									...
									if (!fieldInput) {
										result.comments.push(
											printError("Could not find input inside div.form-field")
										);
									} else {
										result.score += 0.5;
		...
		},
		{
			...
			maxScore: 4,
			run: async () => {
				let result = { id: 5.1, score: 0, comments: [] };
				let cssTag = document.querySelector(
					'link[href="style.css"],link[href="/style.css"],link[href="./style.css"]'
				);
				if (!cssTag) {
					cssTag = document.querySelector(
						'link[href="http://localhost:3000/style.css"],link[href="https://localhost:3000/style.css"],link[href="//localhost:3000/style.css"]'
					);
					if (cssTag) {
						result.comments.push(
							printError("style.css is included using absolute URL")
						);
					}
				} else {
					result.score += 1;
				...
				if (!appView) {
					...
					if (document.body.clientWidth < 1024) {
						await new Promise((resolve, reject) => {
							...
							setTimeout(() => {
								...
								reject(
									new Error(
										"Did not observe change in body width (current body width = " +
					...
					widthPercent = Math.round(
						(100 * rect.width) / document.body.clientWidth
					);
					if (widthPercent === 80) {
						result.score += 1;
						printOK(
							"width of div#app-view is 80% of document body when body width is greater than or equal to 1024"
						);
					} else {
						result.comments.push(
							printError(
								"width of div#app-view is not 80% of document body when body width is greater than or equal to 1024"
							)
						);
					}
					await new Promise((resolve, reject) => {
					...
					if (widthPercent === 90) {
						result.score += 1;
						printOK(
							"width of div#app-view is 90% of document body when body width is greater than or equal to 768 and less than 1024"
						);
					} else {
						result.comments.push(
							printError(
								"width of div#app-view is not 90% of document body when body width is greater than or equal to 768 and less than 1024"
							)
						);
					}
					await new Promise((resolve, reject) => {
					...
					if (widthPercent === 100) {
						result.score += 1;
						printOK(
							"width of div#app-view is 100% of document body when body width is less than 768"
						);
					} else {
						result.comments.push(
							printError(
								"width of div#app-view is not 100% of document body when body width is less than 768"
							)
						);
					}
				}
		...
		},
		{
			...
			maxScore: 2,
			run: async () => {
				let result = { id: 5.2, score: 0, comments: [] };
				if (
					window.location.pathname !== "/chat.html" &&
					window.location.pathname !== "/chat"
				) {
				...
				let otherMessage = document.querySelector(
					".message-list > .message:not(.my-message)"
				);
				if (myMessage && otherMessage) {
					...
					if (myRect.left > otherRect.left && myRect.right > otherRect.right) {
						result.score += 1;
						printOK(
							"My message box appears to be on the right side of friend's message box"
						);
					} else {
						myRect = myMessage
							.querySelector(".message-text")
							.getBoundingClientRect();
						otherRect = otherMessage
							.querySelector(".message-text")
							.getBoundingClientRect();
						if (
							myRect.left > otherRect.left &&
							myRect.right > otherRect.right
						) {
							result.score += 1;
							printOK(
								"My message box appears to be on the right side of friend's message box"
							...
							result.comments.push(
								printError(
									"My message box should be toward the right of friend's message box"
								)
							);
						}
					}
					...
					let otherColor =
						window.getComputedStyle(otherMessage).backgroundColor;
					if (myColor === otherColor) {
						myColor = window.getComputedStyle(
							myMessage.querySelector(".message-text")
						).backgroundColor;
						otherColor = window.getComputedStyle(
							otherMessage.querySelector(".message-text")
						).backgroundColor;
						if (myColor === otherColor) {
							result.comments.push(
								printError(
									"My message box and friend's message box should have different background colors"
								)
							);
						} else {
							result.score += 1;
							printOK(
								"My message box and friend's message box have different background colors"
							);
						}
					} else {
						result.score += 1;
					...
					result.comments.push(
						printError(
							"Could not find message boxes with classes .my-message and .message"
						)
					);
				}
				return result;
		...
		},
		{
			...
			maxScore: 1,
			run: async () => {
				let result = { id: 6, score: 0, comments: [] };
				let screenDevice;
				try {
					...
					console.log(err);
					result.comments.push(
						print("Could not test due to insufficient permissions")
					);
					return result;
				}
	...
	const captureElemScreenshot = async (elemToCapture, videoStream, result) => {
		...
		confirmBox.style.left =
			document.body.clientWidth / 2 - elemRect.width / 2 + "px";
		confirmBox.style.top =
			document.body.clientHeight / 2 - elemRect.height / 2 + "px";
		...
		await delay(1e3);
		alert(
			"Move your mouse to the red box, and hold the cursor over the pixel you want the tester to examine"
		);
		await new Promise((resolve, reject) => {
			let originalBorder = elemToCapture.style.border;
			let blink = setInterval(() => {
				elemToCapture.style.border =
					elemToCapture.style.border === "3px dashed red"
						? originalBorder
						: "3px dashed red";
			}, 500);
			let pixelPos = {};
			...
			let handler = async (evt) => {
				...
				if (
					originalPixel.data[0] !== hoverPixel.data[0] ||
					...
					result.comments.push(
						printError("Could not observe change in color on mouse hover")
					);
				}
				resolve(null);
			};
	...
	const print = (text, ...extra) => (
		store.options.showLogs && console.log("[34m[Tester][0m", text, ...extra), text
	);
	const printError = (text, ...extra) => (
		store.options.showLogs &&
			console.log(
				"[34m[Tester][0m %c Bug " + emoji.bug + " ",
	...
	const printOK = (text, ...extra) => (
		store.options.showLogs &&
			console.log(
				"[34m[Tester][0m %c OK " + emoji.like + " ",
				"background-color: green; color: white; padding: 1px;",
				text,
				...extra
			),
		text
	...
	showLogsCheckbox.checked =
		"showLogs" in store.options ? store.options.showLogs : true;
	...
	checkBoxAll.checked =
		store.selection && Object.keys(store.selection).length > 0
			? Object.values(store.selection).reduce((acc, val) => acc && val, true)
			: false;
	...
	if (store.lastTestAt) {
		renderResult();
		lastTested.textContent =
			"Last Run at: " + new Date(store.lastTestAt).toLocaleString();
	}
	tests.forEach((test, i) => {
		...
		checkBox.checked =
			test.id in store.selection ? store.selection[test.id] : false;
		...
		td3.textContent =
			test.id in store.results
				? store.results[test.id].skipped
					? "-"
					: store.results[test.id].score + "/" + test.maxScore
				: "-";
		ui[test.id] = { checkBox: checkBox, resultCell: td3 };
	});
	...
	runButton.addEventListener("click", async (evt) => {
		...
		await forEachAsync.call(tests, async (test) => {
			...
			if (input.checked) {
				...
				try {
					...
					print(
						"--- Test " +
							test.id +
							" Finished --- Score = " +
							Math.round(100 * result.score) / 100 +
							" / " +
							test.maxScore
					);
				...
				store.results[test.id] = { skipped: true, score: 0, comments: [] };
			}
			cell.textContent = store.results[test.id].skipped
				? "Skipped"
				: Math.round(100 * store.results[test.id].score) / 100 +
				  "/" +
				  test.maxScore;
		});
